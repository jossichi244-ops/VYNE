...
const TimeSeriesChartMockup = memo(({ series }) => {
  ...
  const timeLabelCol =
    Object.keys(series[0]).find((k) => k !== "mean_value") || "Period";
      ...
      <div className="series-summary-grid">
        <div className="summary-item">
          <span className="label">
            Start Value ({series[0][timeLabelCol]}):
          </span>
          <span className="value">{firstVal.toFixed(2)}</span>
        </div>
        <div className="summary-item">
          <span className="label">
            End Value ({series[series.length - 1][timeLabelCol]}):
          </span>
...
function ScenarioAnalysis({ title, data }) {
  ...
  useEffect(() => {
    if (
      !availableFrequencies.includes(activeFrequency) &&
    ...
    <motion.div
      ...
      <motion.div
        ...
        <div className="scenario-column narrative-hypotheses-stack">
          <motion.section
            ...
            <h4 className="scenario-subtitle">
              <BookOpen size={20} className="icon-neon" /> EXECUTIVE NARRATIVES
              (Định Hướng Chiến Lược)
            </h4>
            <p className="narrative-intro">
              Tóm tắt kịch bản, các chuỗi tác động liên hoàn và ý nghĩa chiến
              lược được mô hình tạo ra:
            </p>
            <TruncatedList
              items={narratives}
              initial={5}
          ...
          <motion.section
            className="scenario-block compact-block"
            variants={blockVariants}>
            <h4 className="scenario-subtitle">
              <Lightbulb size={20} className="icon-neon" /> CORE HYPOTHESES (Giả
              Thuyết Tác Động)
            </h4>
            <TruncatedList
              items={hypotheses}
              ...
              renderer={(h) => {
                const text =
                  h["giả_thuyết"] || h["giả thuyết"] || JSON.stringify(h);
                const prob =
                  h["xác_suất"] ?? h["xác suất xảy ra"] ?? h["xác suất"] ?? "";
                return (
        ...
        <div className="scenario-column info-stack">
          ...
          <motion.section
            className="scenario-block time-series-block"
            variants={blockVariants}>
            <h4 className="scenario-subtitle">
              <Clock size={20} className="icon-neon" /> TIME-SERIES TRENDS (Theo
              Thời Gian)
            </h4>
            ...
            {filteredTimeSeries.length === 0 &&
              availableFrequencies.length > 0 && (
                <div className="empty-state">
                  ...
                  {freqMap[activeFrequency] || activeFrequency}** frequency.
                </div>
              )}
            {availableFrequencies.length === 0 && (
              <div className="empty-state">
        ...
        <motion.section
          className="scenario-block full-width"
          variants={blockVariants}>
          <h4 className="scenario-subtitle">
            <TrendingUp size={20} className="icon-neon" /> ESTIMATED IMPACT &
            DATA TRACE
          </h4>

          {/* Bảng Estimated Impact */}
          <div className="table-wrap impact-table-section">
            <h5 className="section-minor-title">
              Variable Impact Scores (Propagation & Correlation)
            </h5>
          ...
          {/* Bảng Data Sample */}
          <h5 className="section-minor-title">
            Scenario Data Sample (Top 5 Records - Traceability)
          </h5>
