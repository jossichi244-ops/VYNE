...
const extractCohortData = (data) => {
  if (!data || data.error || !data.cohort_matrix || !data.retention_matrix) {
    return {
      ...
      narrative: data?.error
        ? `Error: ${data.error}`
        : "Cohort requires proper data.",
    };
  }
  ...
  cohortKeys.forEach((ck) => {
    ...
    cohortDates.forEach((date, idx) => {
      ...
      const retention =
        baseUsers > 0 ? Math.round((users / baseUsers) * 100) : 0;
  ...
  const all100 =
    flatRows.length > 0 && flatRows.every((r) => r.retention === 100);
  const warningNote = all100
    ? " ⚠️ Warning: All retention values are 100%, check data quality."
    : "";

  const narrative = `Analyzed **${totalUsers.toLocaleString()}** users across ${
    validCohorts.length
  } cohort(s), reflecting actual decay over time.${warningNote}`;
...
const CohortAnalysisCard = memo(({ data }) => {
  ...
  if (!model.success) {
    return (
      <motion.div
        className="cohort-card analysis-error"
          ...
          <div className="metric-summary">
            <div className="metric-block">
              <p className="metric-label">
                <User size={16} /> Total Users
              </p>
              <h4 className="metric-value">
                {model.total_users.toLocaleString()}
              </h4>
            </div>
            <div className="metric-block">
              <p className="metric-label">
                <Calendar size={16} /> Cohort Count
              </p>
              <h4 className="metric-value">{model.cohortKeys.length}</h4>
            </div>
          </div>

          <h4 className="panel-title table-title">
            Detailed Retention Matrix (Flattened)
          </h4>
          <motion.div
            className="retention-table-container"
              ...
              <tbody>
                {model.rows.map((row, idx) => (
                  <tr key={idx}>
                    ...
                    <td
                      className={`retention-cell ${getRetentionColor(
                        row.retention
                      )}`}>
                      {row.retention}%
                    </td>
