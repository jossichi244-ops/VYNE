...
import { AlertTriangle, CheckCircle, Target, HelpCircle, BarChart3, Sigma, Users, BarChart } from 'lucide-react';
...
const DiDAnalysisCard = ({ data }) => {
    if (!data || data.error) {
        return (
            <div className="did-error-card">
                ...
                <p className="did-error-message">{data?.error || "An unknown error occurred."}</p>
            </div>
        );
    }
    ...
    const chartData = (!pre_trends || !post_trends) ? [] : [...pre_trends, ...post_trends].map(d => ({
        ...d,
        // Format ngày tháng cho trục X
        time: new Date(d.time).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
    }));
    
    const treatmentTime = (pre_trends && pre_trends.length > 0)
    ...
    return (
        <div className="analysis-card did-card">
            <div className="card-header">
                ...
                <p className="card-subtitle">Evaluating the Causal Impact of the Intervention.</p>
            </div>
            ...
            <div className="metric-grid">
                ...
                />
                <MetricCard 
                    icon={isSignificant ? <CheckCircle size={18} /> : <HelpCircle size={18} />}
                    title="P-value"
                    value={did_pvalue?.toFixed(4) ?? 'N/A'}
                    subtext={isSignificant ? 'Statistically Significant (p < 0.05)' : 'Not Significant'}
                    className={isSignificant ? "card-status-success" : "card-status-warning"}
                />
                <MetricCard 
            ...
            {/* Interpretation Box */}
            <div className={`interpretation-box ${pre_trends_warning ? 'interpretation-warning' : 'interpretation-success'}`}>
                <div className="interpretation-header">
                    {pre_trends_warning ? <AlertTriangle className="interpretation-icon" /> : <CheckCircle className="interpretation-icon" />}
                    <h4 className="interpretation-title">Model Interpretation & Pre-Trend Check</h4>
                </div>
                <p className="interpretation-narrative">{interpretation}</p>
                {pre_trends_warning && (
                    <p className="pre-trend-sub-warning">
                        <BarChart className="sub-warning-icon"/> **Warning:** The assumption of **Parallel Trends** might be violated. Interpret with caution.
                    </p>
                )}
            </div>
                ...
                <div className="chart-container">
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                            ...
                            <XAxis dataKey="time" className="chart-axis" tickLine={false} axisLine={false} />
                            ...
                            <Tooltip 
                                ...
                                labelStyle={{ fontWeight: 'bold', color: 'var(--accent-color)' }}
                            />
                            <Legend wrapperStyle={{fontSize: "13px", paddingTop: "10px"}} align="right" verticalAlign="top" />
                            <Line type="monotone" dataKey="treatment_mean" name="Treatment Group Mean" className="line-treatment" strokeWidth={3} dot={{ r: 3 }} activeDot={{ r: 6 }} />
                            <Line type="monotone" dataKey="control_mean" name="Control Group Mean (Counterfactual)" className="line-control" strokeWidth={3} />
                            {treatmentTime && <ReferenceLine x={treatmentTime} className="reference-line-treatment" strokeDasharray="6 6" label={{ value: 'Treatment Start', position: 'insideTopRight', fill: 'var(--warning-color)', fontSize: 11, fontWeight: 'bold' }} />}
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            </div>
...
export default function BeyondEDAViewer({ apiData }) {
    ...
    const { analysisItems, scenarioCount } = useMemo(() => {
        ...
        analysisKeys.forEach(key => {
            ...
            const isImpactAssessment = key === 'impact_assessment' || 
            ...
            if (isDiD || isCohort || isImpactAssessment) {
                items.push({
                    uniqueKey: key, 
                    title: isDiD ? "DiD Analysis" : isCohort ? "Cohort Analysis" : "Impact Assessment",
                    data: data,
                    isSpecialized: true,
                    isDiD: isDiD,
                    isCohort: isCohort,
            ...
            else if (key === 'scenario' && typeof data === 'object' && data !== null && !data.error) {
        ...
        >
            <motion.header className="viewer-header" variants={itemVariants}>
                ...
                <p className="summary">
                    ...
                    Found <span className="highlight">{scenarioCount}</span> Major Scenario(s) 
                    and <span className="highlight">{analysisItems.length - scenarioCount}</span> Supporting Analyses.
                </p>
            </motion.header>

            <div className="analysis-grid">
                {/* ĐÃ SỬA LỖI: Thêm 'isImpact' vào destructure */}
                {analysisItems.map(({ uniqueKey, title, data, isScenario, isDiD, isCohort, isImpact }) => (
                    <motion.div
                        key={uniqueKey}
                        className={`analysis-item ${isScenario || isDiD || isCohort || isImpact ? "full-width-card" : "half-width-card"}`}
